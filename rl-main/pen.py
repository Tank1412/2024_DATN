...
class PPOPenaltyAgent:
    def __init__(self, input_dims, n_actions, gamma=0.99, lr=0.0003, betas=(0.9, 0.999), kl_beta=1.0, kl_target=0.01, K_epochs=4, device='cpu'):
    ...
    def update(self, memory):
        for _ in range(self.K_epochs):
            ...
            for log_prob, reward, advantage in zip(old_log_probs, rewards, advantages):
                ...
                loss = -surr1 + 0.5*self.MseLoss(states, rewards) + self.kl_beta * kl_div

                self.optimizer.zero_grad()
                loss.backward()
                self.optimizer.step()

                if kl_div > self.kl_target:
                    self.kl_beta *= 1.5
        ...
        while step <= steps:
            ...
            for junction_number, junction in enumerate(all_junctions):
                ...
                if traffic_lights_time[junction] == 0:
                    ...
                    action, log_prob, _ = brain.choose_action(state)
                    phaseDuration(junction, min_duration + 16, select_lane[action][1])
